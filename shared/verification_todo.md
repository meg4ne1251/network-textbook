...
| B2 | `04_ipv6/01_why_ipv6.md` | (a) RIR枯渇日付(APNIC 2011-04, RIPE NCC 2019-11ほか) (b) IPv6到達可能率「半分近く」 (c) 移転市場「1アドレス数十ドル」 | Google IPv6統計 / NRO・各RIR | 済(うち(b)更新) — (a) IANA 2011-02-03 / APNIC 2011-04 / RIPE NCC 2019-11 は歴史的事実で正確。(b) **要更新**: Google統計は2026-03-28に初めて50%突破(50.10%)。「半分近く」→「およそ半数/50%超」に更新、APNIC計測40%台の併記も追加。(c) 2026年の移転相場は$18〜45(平均約$25)、ピーク$50超。「数十ドル規模」は妥当で修正不要 |
| B3 | `04_ipv6/02_addressing.md` | ドキュメント用 3fff::/20(RFC 9637, 2024追加)の記載 | RFC 9637 | 済(確認・修正不要) — RFC 9637「Expanding the IPv6 Documentation Space」がIANAに 3fff::/20 を予約(2024-07-23)、RFC 3849 の 2001:db8::/32 を補完。本文の表記「2001:db8::/32、3fff::/20 → RFC 3849、RFC 9637」は正確 |
| B4 | `04_ipv6/03_ndp_slaac.md` | (a) 一時アドレス寿命 TEMP_PREFERRED_LIFETIME=1日 (b) NUD既定値(BaseReachableTime 30s, DELAY 5s, ユニキャストNS 3回) | RFC 8981 / RFC 4861 | 済(確認・修正不要) — (a) 本文は TEMP_PREFERRED_LIFETIME の具体値を明示していない(RFC 8981の既定1日は事実として正しいが本文に記載なし)。(b) RFC 4861 §10 の定数と全一致: REACHABLE_TIME=30s・MIN/MAX_RANDOM_FACTOR=0.5/1.5(本文「30秒×乱数0.5〜1.5」)、DELAY_FIRST_PROBE_TIME=5s、RETRANS_TIMER=1000ms=1s、MAX_UNICAST_SOLICIT=3(本文「1秒間隔で最大3回」)。修正不要 |
...
個別に確認しておくと安心な数値(Cへ入れたがA寄り)【済 2026-07-13】:
- `06_redundancy/01`: Message Age 1秒/ホップ加算、TC保持 Max Age + Forward Delay = 35秒(802.1D条文)
  → 済(修正不要)。Max Age 20 + Forward Delay 15 = 35 秒、Message Age ホップ加算、Config BPDU 35 オクテットは 802.1D 既定と一致。
- `06_redundancy/02`: TC While タイマー「HelloTime + 1秒」系(802.1Q条文)
  → 済(修正不要)。本文は硬い数値を断定せず「数 Hello 送り続ける」とhedge。RSTP の newTcWhile は HelloTime+1 秒で概ね整合。
- `06_redundancy/04`: LACPDU固定長110オクテット、WAITING約2秒(802.1AX条文)
  → 済(修正不要)。LACPDU 110 オクテット(1+1+20+20+16+2+50)、Aggregate_Wait_Time 2.0 秒、fast 1s/3s・slow 30s/90s は 802.1AX と一致(IEEE 全文は非公開のため算術検算+二次情報で確認)。
- `03_bgp/04`: 空/未定義の prefix-list・community-list 参照時のFRR挙動
  → 済(修正不要)。本文(414–417行)が「実装差がある(全マッチ扱いと全不一致扱いの両方が存在)」と適切にhedge済みで断定していない。

**Tier C 通し確認【済 2026-07-13】**: 上記A寄り数値4件は本文と一致(修正不要)。
実装依存の出力例(show/tcpdump/tc/config)は全章で「代表的な形」「バージョンにより微差」
「Linux の … での例」「FRRouting での骨格」等として実装を明示した例に枠付けされており、
免責の語彙は章ごとに差はあるが「特定実装の具体例=規範ではない」という趣旨は一貫。
0ヒット章(03_bgp/03・05_mpls_srv6/02・03・07_qos/04)も文脈的免責を確認済みで個別修正不要。
...
**Tier D 通し確認【済 2026-07-13】**: (1) 未リンク化の「(後述: …)」プレースホルダは
本文に残存ゼロ(検出2件は verification_todo.md / style_guide.md 自身の記述)。
(2) 全 `.md` の相対リンク 682 本がすべて実在ファイルに解決(自動チェック済み。
別途 5 件が誤検出されたが、いずれも Mermaid 図ラベル `[..](..)` と ASCII パケット図
`Segment List[0](..)` で markdown リンクではない)。リンク切れなし。
