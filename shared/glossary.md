...
- **定義**: STP がブリッジ間で交換する制御メッセージ。木の計算に使う Configuration BPDU(ルートブリッジ ID・ルートパスコスト・送信ブリッジ ID・ポート ID とタイマー3種を運ぶ)と、トポロジ変更をルートへ通知する TCN BPDU の2種がある。宛先はブリッジが転送しない予約グループアドレス 01:80:C2:00:00:00 で、必ず隣接ブリッジまでしか届かず、各ブリッジが自分の BPDU を再生成して伝搬する。タグなしで送受信される。
- **初出章**: `06_redundancy/01_stp_basics.md`(言及は `02_vlan_vxlan_evpn/02_trunking_native_vlan.md` が先行)
- **関連用語**: STP、ルートブリッジ、ルートポート / 指定ポート
...
- **関連用語**: BPDU、ルートブリッジ、ルートポート / 指定ポート、ブロードキャストストーム、透過的ブリッジング
