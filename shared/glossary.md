...
- **定義**: 4つの独立なクラス × 3段階の廃棄優先度の行列として定義される PHB 群(RFC 2597)。AFxy と表記し(x = クラス、y = 廃棄優先度)、DSCP 値は 8x + 2y で計算できる(AF31 = 26)。クラス間は別キュー・別資源配分の独立な席、クラス内の廃棄優先度は輻輳時の捨てられやすさ(AFx3 から先に捨てる)を表す。契約超過分を捨てずに廃棄優先度を上げて通す降格マーキング(三色マーカー)と、優先度を読んで選択的に捨てる WRED の基盤になる。
- **初出章**: `07_qos/02_classification_marking.md`
- **関連RFC**: RFC 2597
- **関連用語**: DSCP、PHB、EF、DiffServ、RED / WRED、三色マーカー
...
- **定義**: キューが満杯になってから機械的に捨てる(テールドロップ)のではなく、持続的なキューの成長を検知して満杯前に確率的に廃棄(または ECN マーク)し、送信者へ意図的に輻輳信号を送るキュー管理の総称。テールドロップの3病理(満杯キューの恒常化・ロックアウト・グローバル同期)を治す。RFC 7567(BCP 197、旧 RFC 2309)がすべてのルータでの導入と、自動チューニング型(CoDel、PIE)の使用を勧告する。古典は RED。
- **初出章**: `07_qos/03_queuing_scheduling.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 7567(旧 RFC 2309)
- **関連用語**: テールドロップ、RED / WRED、CoDel / fq_codel、バッファブロート、輻輳、ECN
...
- **定義**: STP がブリッジ間で交換する制御メッセージ。木の計算に使う Configuration BPDU(ルートブリッジ ID・ルートパスコスト・送信ブリッジ ID・ポート ID とタイマー3種を運ぶ)と、トポロジ変更をルートへ通知する TCN BPDU の2種がある。宛先はブリッジが転送しない予約グループアドレス 01:80:C2:00:00:00 で、必ず隣接ブリッジまでしか届かず、各ブリッジが自分の BPDU を再生成して伝搬する。タグなしで送受信される。RSTP/MSTP では TCN は(互換用途を除き)廃止され、Flags の 8 ビットをすべて使う RST/MST BPDU(Type 0x02)に統合された。
- **初出章**: `06_redundancy/01_stp_basics.md`(言及は `02_vlan_vxlan_evpn/02_trunking_native_vlan.md` が先行)
- **関連用語**: STP、RSTP、ルートブリッジ、ルートポート / 指定ポート
...
### CoDel / fq_codel(Controlled Delay / FlowQueue-CoDel)

- **定義**: キューの長さではなくパケットの滞在時間(sojourn time)を測り、target(既定 5ms)を interval(既定 100ms)にわたって下回れない定在キューだけを先頭から廃棄する自動チューニング型の AQM(RFC 8289)。廃棄間隔を interval/√n で詰めて圧を強める。リンク速度・RTT・フロー数を入力に要しない点が RED のチューニング問題への回答。fq_codel(RFC 8290)は 5 タプルハッシュのフローキュー群(既定 1024)を DRR で公平に回し(新規フローを1巡優遇)、各キューへ独立に CoDel を適用した合成で、Linux の代表的な既定 qdisc。
- **初出章**: `07_qos/03_queuing_scheduling.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 8289、RFC 8290
- **関連用語**: AQM、DRR、バッファブロート、テールドロップ
...
- **定義**: フローを少数のクラスに集約し、エッジで分類・マーキング(トラフィック条件付け)を行い、コアはヘッダのマーク(DSCP)だけを見てクラスごとの転送動作(PHB: Per-Hop Behavior)を機械的に適用する QoS アーキテクチャ(RFC 2475)。フローごとの状態をコアに持たせず「状態はパケット自身が荷札として運ぶ」ため大規模網にスケールし、事実上の標準となった。一貫したポリシーが及ぶ範囲を DiffServ ドメインと呼び、その縁が信頼境界になる。
- **初出章**: `07_qos/01_qos_basics.md`
- **関連RFC**: RFC 2475、RFC 2474
- **関連用語**: IntServ、QoS、FEC、輻輳、DSCP、PHB、信頼境界
...
- **定義**: キューごとにバイト建ての持ち点(quantum)と繰り越し残高(deficit counter)を持たせ、番が来たら残高の範囲で送り、使い残しを次巡へ繰り越すラウンドロビン。パケット長の違いによる WRR の不公平を解消し、長期の送出バイト数を quantum の比に収束させる重み付き公平スケジューラ。加減算だけで動き、quantum ≥ MTU なら1巡 O(1) — ソートを要する WFQ に対する実装の本命であり、fq_codel(RFC 8290)のフロー間スケジューラにも採用されている(DRR 自体は 1995 年の Shreedhar–Varghese の論文に由来し、RFC ではない)。
- **初出章**: `07_qos/03_queuing_scheduling.md`
- **関連RFC**: RFC 8290(採用例)
- **関連用語**: WFQ、優先キューイング、CoDel / fq_codel、QoS

### DSCP(Differentiated Services Codepoint)

- **定義**: IPv4/IPv6 ヘッダの DS フィールド(旧 ToS オクテット、IPv6 では Traffic Class)の上位6ビットに書かれる、パケットの所属クラスを示す荷札(RFC 2474。残り2ビットは ECN)。64通りの値の名前空間であり、値と転送動作(PHB)の対応はドメインごとに決めてよいが、実務は推奨コードポイント(EF = 46、CS6 = 48 等)に従う。tcpdump 等の tos 表示は8ビット全体の16進値で、DSCP はその上位6ビット(ToS バイト = DSCP × 4)。送り手の自己申告であるため、信頼境界での検認(信頼・再マーキング・洗い流し)と組で機能する。
- **初出章**: `07_qos/02_classification_marking.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 2474
- **関連用語**: PHB、DiffServ、クラスセレクタ、AF、EF、信頼境界、PCP
...
- **初出章**: `01_fundamentals/02_routing_table_basics.md`
- **関連RFC**: RFC 2992
- **関連用語**: メトリック、リンクアグリゲーション

### ECN(Explicit Congestion Notification / 明示的輻輳通知)

- **定義**: AQM の廃棄判定を「捨てる」から「印を付けて通す」に置き換え、パケットを壊さずに損失1個と同等の輻輳信号を送信者へ届ける仕組み(RFC 3168)。DS フィールドの下位2ビットを使い、送信者が ECT(01/10)で対応を宣言し、輻輳点の AQM が CE(11)へ書き換え、受信者がトランスポート層(TCP では ECE/CWR フラグ)で送信者へエコーする。CE を打てるのは AQM だけであり、Not-ECT のパケットは従来どおり廃棄で信号する。信頼境界で DSCP を洗い流す際に下位2ビットまで潰す(bleaching)と経路上の恩恵が失われる。ECT(1) を識別子に再利用する発展形が L4S(RFC 9330〜9332)。
- **初出章**: `07_qos/04_shaping_policing.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 3168、RFC 6040、RFC 9330
- **関連用語**: AQM、DSCP、CoDel / fq_codel、輻輳

### EF(Expedited Forwarding / 優先転送)

- **定義**: 低遅延・低ジッタ・低損失 — 仮想的な専用線相当の扱いを目指す PHB(RFC 3246、旧 RFC 2598。推奨 DSCP は 46 = 101110、ToS 表示では 0xb8)。「EF のキューは常にほぼ空」を保つことが実装の狙いで、厳密優先の席が他クラスを飢えさせないよう流入のポリシングとセットで運用することが仕様の前提。VoIP 音声の標準的な荷札である。
- **初出章**: `07_qos/02_classification_marking.md`
- **関連RFC**: RFC 3246
- **関連用語**: DSCP、PHB、AF、QoS、優先キューイング、シェーピング / ポリシング
...
### FHRP(First Hop Redundancy Protocol / ファーストホップ冗長化プロトコル)

- **定義**: ホストのデフォルトゲートウェイ(最初の L3 ホップ)を複数ルータで冗長化するプロトコルの総称(VRRP、HSRP、GLBP)。仮想 IP + 仮想 MAC の組(仮想ルータ)を実ルータから独立した「役」として定義し、生きているルータの1台がそれを演じることで、ホストの設定・ARP キャッシュを一切変えずに切り替える。『FHRP』という総称自体は IETF/IEEE の仕様には登場しない業界慣用名(Cisco の文書に由来)。
- **初出章**: `06_redundancy/03_fhrp_vrrp.md`
- **関連用語**: VRRP、HSRP、仮想ルータ、デフォルトゲートウェイ、エニーキャストゲートウェイ
...
- **定義**: TTL(IPv6 では Hop Limit)を最大値の 255 で送信し、受信側が「TTL がほぼ 255 のまま届いたか」を検査することで、遠隔からの偽造パケットを排除する保護機構。TTL は転送で減ることはあっても増えることはないため、ネットワークの遠くにいる攻撃者には原理的に偽造できない。直結 eBGP ピアの保護が代表的な用途で、VRRP の広告検査(TTL 255 必須)も同原理である。
- **初出章**: `03_bgp/02_ibgp_ebgp.md`
- **関連RFC**: RFC 5082
- **関連用語**: TTL、eBGP / iBGP、VRRP
...
- **定義**: Cisco 独自の FHRP(RFC 2281 は標準ではなく既存実装を文書化した Informational)。VRRP に先行して 1990 年代半ばから使われた。骨格(仮想 IP + 仮想 MAC を1台が演じる)は VRRP と同一で、役の呼称は Active / Standby、UDP 1985・224.0.0.2、既定 Hello 3 秒 / Hold 10 秒、仮想 MAC 00:00:0C:07:AC:{グループ}、プリエンプションは既定で無効、仮想 IP は実アドレスと別に必須、という細部が異なる。
- **初出章**: `06_redundancy/03_fhrp_vrrp.md`
- **関連RFC**: RFC 2281(Informational)
- **関連用語**: FHRP、VRRP、プリエンプション
...
- **定義**: 品質を求めるフローごとに、RSVP(RFC 2205)で経路上の全ルータへ帯域予約を行い、定量的な品質保証を与える QoS アーキテクチャ(RFC 1633)。予約はソフトステートとして周期リフレッシュで維持される。コアが持つ状態量がフロー数に比例して爆発するためインターネット規模では採用されず、RSVP はトラフィックエンジニアリング(RSVP-TE)の道具として存続した。
- **初出章**: `07_qos/01_qos_basics.md`
- **関連RFC**: RFC 1633、RFC 2205
- **関連用語**: DiffServ、QoS、RSVP-TE、ソフトステート
...
- **定義**: リンクアグリゲーションの束を両端の明示的な合意によって構築・維持する制御プロトコル(IEEE 802.1AX)。各物理リンク上で LACPDU(宛先 01:80:C2:00:00:02、EtherType 0x8809 = Slow Protocols)を交換し、「全メンバーリンクで同一のシステム ID・キーの相手が見えること」を束ねる条件とする(誤配線されたリンクは束から自動除外される)。LACPDU は心拍を兼ね、途絶(fast: 3 × 1 秒 / slow: 3 × 30 秒)はリンクを束から外す方向 — フェイルクローズ — に倒れる。
- **初出章**: `06_redundancy/04_lag_mlag.md`
- **関連用語**: リンクアグリゲーション、MLAG
...
### MAC フラッピング(MAC flapping)

- **定義**: 同一の送信元 MAC アドレスが複数のポート(または VTEP)から交互に観測され、MAC アドレステーブルの学習結果が高頻度で書き換わり続ける状態。L2 ループの代表的な兆候で、テーブルが安定しないためユニキャストの誤配送を伴う。EVPN はこれをシーケンス番号つきの MAC モビリティとして制御し、急増を重複 MAC 検出として凍結する。
- **初出章**: `06_redundancy/01_stp_basics.md`
- **関連用語**: ブロードキャストストーム、MAC モビリティ、透過的ブリッジング
...
- **定義**: 2台のスイッチがピアリンクで状態(MAC 学習・メンバー状態・設定整合性)を同期し、LACP 上で共通のシステム ID を名乗ることで、対向(サーバや別スイッチ)から1台に見える LAG を2シャーシにまたがって提供する構成。リンク冗長と装置冗長を両立し、装置障害を対向のリンク断として即時処理させる。『MLAG』という総称を含め IEEE/IETF の標準は存在しないベンダー独自技術(vPC、MC-LAG 等。同期チャネルの部分標準に ICCP = RFC 7275、不発の標準化に 802.1AX-2014 の DRNI)。標準による代替は EVPN マルチホーミング(ESI-LAG、RFC 7432)。
- **初出章**: `06_redundancy/04_lag_mlag.md`
- **関連RFC**: RFC 7275(ICCP)、RFC 7432(EVPN マルチホーミング)
- **関連用語**: リンクアグリゲーション、LACP、ピアリンク、ESI、スプリットブレイン
...
- **関連用語**: FEC、LSP、LSR / LER、LFIB、PHP、ラベルスタック、カプセル化、セグメントルーティング

### MSTP(Multiple Spanning Tree Protocol)

- **定義**: VLAN 群を少数のインスタンス(MSTI)へ多対一に写像し、インスタンスごとに別の形の木を張って冗長リンクを VLAN 群単位で使い分けるスパニングツリー(IEEE 802.1s として登場、802.1Q-2005 以降本体に統合)。BPDU はリンクごとに1本で、CIST の情報に全 MSTI の情報(各 16 オクテットの MSTI Configuration Message)が相乗りするため、制御負荷が VLAN 数に比例しない(PVST+ との違い)。写像の一致範囲は MST リージョンとして管理され、リージョン間・非 MSTP ブリッジとの間は CIST(全体で1本の木)で接続される。リージョン内の鮮度管理は Message Age ではなく残りホップ数(既定 20)。
- **初出章**: `06_redundancy/02_rstp_mstp.md`(言及は `06_redundancy/01_stp_basics.md` が先行)
- **関連用語**: MST リージョン、RSTP、STP、VLAN、FEC

### MST リージョン(MST region)

- **定義**: MSTP で VLAN→インスタンス写像を共有できるブリッジの範囲。リージョン名(32 オクテット)・リビジョン番号(2 オクテット)・VLAN→インスタンス対応表の HMAC-MD5 ダイジェスト(16 オクテット)の3点(MST Configuration Identifier)が BPDU 上で完全一致したときだけ同一リージョンとみなされる。1 VLAN でも写像がずれるとダイジェスト不一致で別リージョンに分裂し、境界では CIST しか共有されない(MSTI の負荷分散が無効になる)。外から見ると1つのリージョンは1台の仮想ブリッジとして振る舞う。
- **初出章**: `06_redundancy/02_rstp_mstp.md`
- **関連用語**: MSTP、RSTP
...
- **定義**: 802.1Q タグの TCI 先頭3ビットの L2 優先度フィールド(「CoS」は Cisco 系の慣用名、「802.1p」はこれを導入した 1990 年代の 802.1D 追補プロジェクトの名で、現在は 802.1Q 本体が規定)。8値の序列には 1(Background)が 0(Best Effort、既定)より低いという折れ曲がりがある。タグの中にしか存在しないため、タグなし区間・L3 ホップを越えられないリンクローカルな荷札であり、IP ヘッダを見ない L2 機器のために DSCP の上位3ビットから区間ごとに再生成する要約と位置づける。隣の DEI ビットは廃棄適格の印(AF の廃棄優先度の L2 版)。
- **初出章**: `07_qos/02_classification_marking.md`(フィールド自体の初出は `02_vlan_vxlan_evpn/01_vlan_basics.md`)
- **関連用語**: 802.1Q、DSCP、QoS、トランクポート

### PHB(Per-Hop Behavior)

- **定義**: DiffServ において、あるクラスのパケットが各ホップで受ける転送動作(スケジューリング・廃棄の扱い)の仕様(RFC 2475)。DSCP という値の名前空間とは分離して標準化されており、値と動作の対応はドメインごとに決められる(実務は推奨コードポイントに従う)。標準の PHB は Default(ベストエフォート)、クラスセレクタ(Precedence 後方互換)、EF(低遅延、ポリシングとセット)、AF(4クラス × 3廃棄優先度)、LE(ベストエフォート未満、RFC 8622)。
- **初出章**: `07_qos/02_classification_marking.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 2474、RFC 2475
- **関連用語**: DSCP、DiffServ、EF、AF、クラスセレクタ
...
- **定義**: 輻輳の瞬間に「どのパケットを先に送り、どのパケットを待たせ、どのパケットを捨てるか」を偶然ではなく意図で決める技術体系。帯域を生む技術ではなくゼロサムの配分であり、輻輳がなければ転送の振る舞いを変えず、一貫したポリシーを敷ける自分のドメインの中でしか効かない。機器内部では「分類→マーキング→ポリシング→キューイング→スケジューリング→シェーピング」のパイプラインとして実装される。
- **初出章**: `07_qos/01_qos_basics.md`
- **関連RFC**: RFC 2475
- **関連用語**: 輻輳、ジッタ、ベストエフォート、DiffServ、IntServ、DSCP、AQM、シェーピング / ポリシング
...
- **定義**: 平均キュー長(EWMA)が min_th を超えたら max_th まで確率を上げながらランダムに早期廃棄する古典的 AQM(1993 年の Floyd–Jacobson の論文に由来し、RED 自体は RFC ではない。RFC 7567 が AQM の代表例として参照)。損失をフローと時刻に散らしてグローバル同期を壊し、キューを浅く保つが、閾値の適値がリンク速度・RTT・フロー数に依存するチューニング問題を抱える。WRED はプロファイルをマークごとに複数持つ拡張で、AF の廃棄優先度(輻輳時に AFx3 から先に捨てる)の執行機構。早期廃棄は輻輳制御を持つ TCP への信号であり、減速しない実時間系(UDP)には適用しない。
- **初出章**: `07_qos/03_queuing_scheduling.md`(言及は `07_qos/02_classification_marking.md` が先行)
- **関連RFC**: RFC 7567、RFC 2597(AF)
- **関連用語**: AQM、AF、テールドロップ、CoDel / fq_codel、ECN、三色マーカー
...
- **定義**: 802.1D のタイマー待ちによる収束(30〜50 秒)を明示的な確認に置き換え、ミリ秒〜秒オーダーの収束を実現したスパニングツリー(IEEE 802.1w として登場、802.1D-2004 で本体に統合され、現在は 802.1Q が規定)。主要機構は、事前計算済みの代替ポートの即時昇格、ポイントツーポイントリンク上の Proposal/Agreement 握手(sync で下流を塞いでから開ける)、全ブリッジによる BPDU 生成と 3 × Hello(既定 6 秒)での障害検知、エッジポートの標準化、TC の直接フラッディングと MAC 即時フラッシュ。802.1D ブリッジを検出したポートは互換モードに落ちる(復帰は手動)。
- **初出章**: `06_redundancy/02_rstp_mstp.md`(言及は `06_redundancy/01_stp_basics.md` が先行)
- **関連用語**: STP、MSTP、BPDU、代替ポート / バックアップポート、エッジポート
...
### SID(Segment Identifier / セグメント識別子)

- **定義**: セグメントルーティングにおける命令(セグメント)の識別子。基本は2種 — Prefix-SID は「そのノードへ IGP の最短経路で行け」というグローバルな命令(ECMP も効く)、Adjacency-SID は「この特定のリンクを渡れ」という広告ノードローカルな命令。両者の組み合わせで任意の経路を「最短経路からの逸脱点」の数だけの命令で表現できる。SR-MPLS では MPLS ラベル(インデックス+ SRGB)、SRv6 では 128 ビットの IPv6 アドレス(Locator + Function + Argument 構造、RFC 8986)として実装される。IGP の拡張(OSPF = RFC 8665、IS-IS = RFC 8667)で配布される。
- **初出章**: `05_mpls_srv6/04_srv6.md`
- **関連RFC**: RFC 8402、RFC 8986、RFC 8665、RFC 8667
- **関連用語**: セグメントルーティング、SRv6、SRH、IGP
...
- **定義**: SRv6 のセグメントリストを運ぶ IPv6 Routing 拡張ヘッダ(Routing Type 4、RFC 8754)。セグメントリストは逆順に格納され(Segment List[0] が最後の経由地)、Segments Left が現在位置のポインタを兼ねる — 現在のセグメントは常に宛先アドレスに置かれ、SID の持ち主が命令を実行して Segments Left を減らし次のセグメントを宛先へ書き込む。単一の SR ドメインに閉じる前提で、境界では外来の SRH 付きパケットをフィルタする(RH0 廃止 = RFC 5095 の反省)。固定部 8 オクテット+ SID 1つにつき 16 オクテット。
- **初出章**: `05_mpls_srv6/04_srv6.md`(言及は `04_ipv6/01_why_ipv6.md` が先行)
- **関連RFC**: RFC 8754
- **関連用語**: SRv6、SID、拡張ヘッダ、IPv6

### SRv6(Segment Routing over IPv6)

- **定義**: セグメントルーティングの IPv6 データプレーン実装。セグメントを 128 ビットの IPv6 アドレス(SID)で表し、リストを SRH に載せる。中継は通常の IPv6 転送そのものなので、対応が必要なのは経由地・終端のノードだけ(RFC 8200 の「拡張ヘッダは宛先だけが処理する」原則の帰結)。RFC 8986 の Locator + Function 構造により End / End.X / End.DT4/DT6 等の「アドレスが命令になる」動作が定義され、MPLS なしで BGP フリーコアと L3VPN を再現できる(BGP による SID 配布は RFC 9252)。代償はヘッダ長(圧縮 SID = RFC 9800 で緩和)と SR ドメインの信頼境界管理。
- **初出章**: `05_mpls_srv6/04_srv6.md`(言及は `04_ipv6/01_why_ipv6.md`・`00_overview/01_roadmap.md` が先行)
- **関連RFC**: RFC 8754、RFC 8986、RFC 9252
- **関連用語**: セグメントルーティング、SRH、SID、IPv6、L3VPN、MPLS

### STP(Spanning Tree Protocol / スパニングツリープロトコル)

- **定義**: 冗長リンクを含む L2 トポロジから全域木(spanning tree)を分散計算で選び、木に含まれないポートをブロッキングしてループを断つプロトコル(IEEE 802.1D。2014 年改訂で 802.1Q に統合)。ルートブリッジ ID → ルートパスコスト → 送信ブリッジ ID → ポート ID の多段比較で全ブリッジが同じ木に合意する。収束は既定タイマーで 30〜50 秒(Max Age 20 秒 + Forward Delay 15 秒 × 2)。高速化した後継が RSTP(802.1w 由来)。
- **初出章**: `06_redundancy/01_stp_basics.md`(言及は第2部が先行)
- **関連用語**: BPDU、RSTP、MSTP、ルートブリッジ、ルートポート / 指定ポート、ブロードキャストストーム、透過的ブリッジング、リンクアグリゲーション
...
- **定義**: セグメントルーティング環境の高速迂回。各ルータが隣接リンク/ノード障害後のトポロジでの最短経路を事前に SPF で計算しておき、障害検知の瞬間に影響パケットへ迂回路のセグメントリストを押し込んで送り出す。バックアップ経路を網に事前設置(RSVP-TE FRR の bypass tunnel)するのではなくパケットに書き込むため状態が不要で、セグメントリストで迂回路を明示できるためどんなトポロジでも保護経路を作れる(従来 LFA = RFC 5286 の限界の解消)。収束中のマイクロループ対策(RFC 8333)も同じ道具立てで行う。
- **初出章**: `05_mpls_srv6/04_srv6.md`(思想の予告は `05_mpls_srv6/02_ldp_rsvp_te.md`)
- **関連RFC**: RFC 5286、RFC 8333
- **関連用語**: Fast Reroute、セグメントルーティング、SPF、収束
...
- **定義**: FHRP の IETF 標準(現行仕様は VRRPv3 = RFC 9568、IPv4/IPv6 両対応)。IP プロトコル番号 112、宛先 224.0.0.18 / ff02::12 で、TTL/Hop Limit 255 検査(GTSM と同原理)により偽広告をリンク内に限定する。メッセージは ADVERTISEMENT の1種類のみで、マスタだけが送信し(既定 1 秒、v3 の単位はセンチ秒)、バックアップは 3 × 広告間隔 + Skew_Time の途絶で交代する。Skew_Time(優先度が高いほど短い)がメッセージ交換なしの選挙として働き、Priority 0 の広告が計画的な辞任を伝える。RFC 9568 は用語 Master を Active に改称した(本書は実装で広く使われるマスタ/バックアップ表記を標準とする)。
- **初出章**: `06_redundancy/03_fhrp_vrrp.md`
- **関連RFC**: RFC 9568(旧 RFC 5798、RFC 3768、RFC 2338)
- **関連用語**: FHRP、HSRP、仮想ルータ、プリエンプション、GTSM
...
### WFQ(Weighted Fair Queuing / 重み付き公平キューイング)

- **定義**: 理想の流体スケジューラ GPS(全キューを重み比で無限に細かく同時に送る思考実験)をパケット単位で近似するスケジューラ(1989 年の Demers らの論文と Parekh–Gallager の解析に由来。IETF 標準ではなく、各ベンダーの「WFQ」はこの系譜の実装群を指す)。「GPS で送っていたらいつ送り終わるか」という仮想終了時刻が最小のパケットを次に送ることで、バイト単位の重み付き公平と遅延保証を得る。代償はパケットごとの最小値選択(O(log n))で、実装の主流は同等の公平を加減算だけで得る DRR に移った。
- **初出章**: `07_qos/03_queuing_scheduling.md`
- **関連用語**: DRR、優先キューイング、QoS
...
- **定義**: 端末しかつながらないとして管理者が宣言した、木の計算の「外」に置かれるポート(RSTP で標準化。ベンダー実装の PortFast に相当)。即座に Forwarding となり、Proposal/Agreement の sync 対象からも外れ、状態遷移してもトポロジ変更(TC)を発火しない。BPDU を受信した瞬間にエッジ資格を失って通常ポートに戻る(スイッチ誤接続への安全装置)。誤ってエッジ宣言を漏らした端末収容ポートは、端末の抜き差しのたびに TC と MAC フラッシュを全網に撒く典型的な事故源になる。
- **初出章**: `06_redundancy/02_rstp_mstp.md`(要求の背景は `06_redundancy/01_stp_basics.md`)
- **関連用語**: RSTP、STP、BPDU
...
- **定義**: VRRP が定義する「VRID + 仮想 IP アドレス + 仮想 MAC アドレス(IPv4: 00:00:5E:00:01:{VRID} / IPv6: 00:00:5E:00:02:{VRID})」の組。実ルータから独立したアイデンティティであり、ホストはこれをデフォルトゲートウェイとして知る。フェイルオーバーで引き継がれるのはこの「名前」だけで状態は引き継がれない(IP 転送が無状態だから成立する)。MAC まで仮想化することで、切替時に修正すべき台帳がホストの ARP キャッシュからスイッチの FDB の1点に縮約される。
- **初出章**: `06_redundancy/03_fhrp_vrrp.md`
- **関連RFC**: RFC 9568
- **関連用語**: VRRP、FHRP、デフォルトゲートウェイ、エニーキャストゲートウェイ
...
### クラスセレクタ(Class Selector / CS)

- **定義**: DSCP のうち下位3ビットが 000 の8値(CS0〜CS7)。上位3ビットの位置が RFC 791 の IP Precedence と一致するように予約された後方互換の系列で、DSCP を解釈しない古い機器にも Precedence として意味が通じる(RFC 2474)。要求される動作は「番号が大きいほど劣らない扱い」という緩やかな順序のみ。CS6(48 = 旧 Internetwork Control、ToS 表示 0xc0)をルーティングプロトコルの荷札とする慣行が代表的な用例。
- **初出章**: `07_qos/02_classification_marking.md`(CS6 の言及は `07_qos/01_qos_basics.md`・`06_redundancy/03_fhrp_vrrp.md` が先行)
- **関連RFC**: RFC 2474
- **関連用語**: DSCP、PHB、DiffServ
...
### 三色マーカー(Three Color Marker / srTCM・trTCM)

- **定義**: メータ(トークンバケット)の判定を green(契約内)/ yellow(超過だが空きがあれば通す)/ red(廃棄)の3値に拡張し、色を AF の廃棄優先度への降格マーキングとして DSCP に書き込む機構。srTCM(RFC 2697)は CIR 1本+2段のバケット(CBS/EBS)で「バーストの深さ」により、trTCM(RFC 2698)は2本のレート(CIR/PIR)で「持続レート」により yellow を決める。入口が付けた色を輻輳点の WRED が執行する(混んだら yellow から捨てる)分業により「空いていれば超過も運ぶ」が完成する。前段の色を尊重し昇格を禁じる color-aware モードがある。
- **初出章**: `07_qos/04_shaping_policing.md`(言及は `07_qos/02_classification_marking.md` が先行)
- **関連RFC**: RFC 2697、RFC 2698
- **関連用語**: トークンバケット、AF、RED / WRED、シェーピング / ポリシング、DSCP

### ジッタ(jitter)

- **定義**: 遅延の揺らぎ。エンドツーエンド遅延の4成分(直列化・伝搬・処理・キューイング)のうち固定成分は揺れないため、その正体はキューイング遅延の変動である。音声などの実時間通信は受信側のジッタバッファで吸収するが、これはジッタを遅延に両替する操作であり、原資は遅延予算(音声なら片道 150ms 目安、ITU-T G.114)から出る。
- **初出章**: `07_qos/01_qos_basics.md`
- **関連用語**: QoS、輻輳

### シェーピング / ポリシング(shaping / policing)

- **定義**: トークンバケットが定義するレート契約の2つの執行流儀。ポリシングはバッファを持たず、超過を即時に廃棄または降格マーキングする(遅延ゼロ。入力側の検認 — 契約の執行、EF の定員 — に使う)。シェーピングは超過をキューに待たせて出力を CIR にならす non-work-conserving な機構で、超過を遅延に両替する(出力側の整形に使う)。定石は「受け取る側はポリス、送る側はシェープ」。ラインレートで背中合わせに吐かれる TCP のバーストとの相性はポリシングが悪く(多重損失 → RTO)、シェーピングが良い。
- **初出章**: `07_qos/04_shaping_policing.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 2475(トラフィック条件付け)
- **関連用語**: トークンバケット、三色マーカー、QoS、バッファブロート、優先キューイング
...
- **定義**: DiffServ ドメインの縁で「どこから先の付けてきたマーク(DSCP/PCP)を信じるか」を定める線。DSCP は送り手の自己申告であり真偽を転送の仕組みが保証しないため、境界の各ポートで信頼・再マーキング(自ドメインの語彙への翻訳)・洗い流し(0 への上書き)のいずれを行うかを設計として決める。定石は「できるだけ端末の近くに、ただし管理の及ぶ範囲の内側に引く」— 端末は信頼せず、管理された機器(IP 電話)は条件付きで信頼し、他ドメインとは契約で合意する。
- **初出章**: `07_qos/02_classification_marking.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 2475
- **関連用語**: DSCP、DiffServ、QoS

### スプリットブレイン(split brain)

- **定義**: 状態や役割を共有する冗長ペアの間の通信(広告・同期)だけが失われ、両系が「相方は死んだ」と判断して同じ役を同時に演じてしまう故障モード。VRRP では2台マスタ(同じ仮想 MAC を2箇所から送出 → MAC フラッピング)、MLAG では同期停止のまま両系が同一システム ID で LACP を維持する形で現れる。対策は、生死判定の経路をデータ同期の経路から分離すること(別経路のキープアライブ)と、衝突時にどちらが降りるかの事前合意。
- **初出章**: `06_redundancy/04_lag_mlag.md`(言及は `06_redundancy/03_fhrp_vrrp.md` が先行)
- **関連用語**: MLAG、VRRP、ピアリンク、MAC フラッピング
...
### セグメントルーティング(Segment Routing / SR)

- **定義**: 経路の知識を網(中間ルータの状態)からパケット(入口が書き込むセグメントリスト)へ移すソースルーティングのアーキテクチャ(RFC 8402)。セグメント(SID)は「命令」であり、Prefix-SID / Adjacency-SID の列で任意の道を中間状態ゼロで表現する。SID は IGP の拡張で配布されるため LDP が不要になり、経路は入口(またはコントローラ)が決めるため RSVP-TE のシグナリングも不要になる。データプレーンは SR-MPLS(ラベルスタック = セグメントリスト、RFC 8660)と SRv6(RFC 8754)の2つ。
- **初出章**: `05_mpls_srv6/04_srv6.md`(言及は `05_mpls_srv6/01〜02` が先行)
- **関連RFC**: RFC 8402、RFC 8660、RFC 8754
- **関連用語**: SID、SRv6、SRH、MPLS、トラフィックエンジニアリング、TI-LFA
...
- **関連用語**: RSVP-TE、ホールドタイム、BGP、IntServ
...
### 代替ポート / バックアップポート(alternate port / backup port)

- **定義**: RSTP が 802.1D の「ブロッキングポート」を役割で2分したもの。代替ポートは他ブリッジの指定ポートから(自分のルートポート経由より劣位の)BPDU を受けているポートで、ルートへの別経路の入り口。ルートポート喪失時にタイマー待ちなしで新ルートポートへ即時昇格する、事前計算済みのバックアップである。バックアップポートは自分自身の指定ポートが面する同一セグメントへの2本目のポート(ハブ経由でしか生じず現代ではまれ)。どちらも状態は Discarding。
- **初出章**: `06_redundancy/02_rstp_mstp.md`
- **関連用語**: RSTP、ルートポート / 指定ポート、フローティングスタティックルート、TI-LFA
...
- **定義**: キューが満杯のとき後着のパケットを捨てる、機器の既定の廃棄戦略。廃棄が満杯後にしか始まらないため、TCP(損失まで加速する)の下では ①満杯キューの恒常化(全パケットが最大キューイング遅延を被り、バースト吸収の余地を失う)、②ロックアウト(キューを埋めた攻撃的フローが守られ、新参が一方的に捨てられる)、③グローバル同期(全フローが同時に減速・回復し、利用率が鋸歯状に振動する)の3病理を生む。RFC 7567(旧 RFC 2309)は AQM への置き換えを勧告する。
- **初出章**: `07_qos/03_queuing_scheduling.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 7567
- **関連用語**: AQM、RED / WRED、バッファブロート、輻輳
...
- **定義**: ホストが自サブネット外宛てのパケットを託すルータ。ホストはゲートウェイのIPをARP/NDPで解決してフレームを送る。ホストは通常1つしか持てず死活も監視しないため単一障害点になる(冗長化は FHRP が担う)。
- **初出章**: `01_fundamentals/01_l2_l3_recap.md`
- **関連用語**: ARP、ホップバイホップ転送、FHRP、VRRP
...
- **定義**: レート契約を「任意の長さ T の窓で送出バイト数 ≤ CIR × T + CBS」という窓の恣意性のない形で定義する機構。バケット(深さ CBS)にトークンが CIR で補充され、パケットは残高があれば適合(conform)、なければ超過(exceed)と判定される。CIR が長期平均の上限、CBS がラインレートで背中合わせに送ってよいバースト許容を表す。判定後の執行(捨てる/降格する/待たせる)がポリシングとシェーピングである。DRR の deficit counter と同じ「バイトの帳簿」であり、貯まる契機が巡回か時間かだけが違う。
- **初出章**: `07_qos/04_shaping_policing.md`
- **関連用語**: シェーピング / ポリシング、三色マーカー、DRR
...
- **定義**: 深すぎるバッファとテールドロップの組み合わせにより、TCP がバッファを満たしきるまで加速し、キューが常時ほぼ満杯 — 数百 ms〜秒級の恒常キューイング遅延 — で運転され続ける病理。帯域(スループット)は出ているのに RTT だけが膨張し、同じ輻輳点を通る音声・対話型のすべてが巻き添えになる。負荷をかけながらの ping と無負荷時の RTT 差(= キュー滞在時間)で切り分け、処方箋は滞在時間ベースの AQM(CoDel / fq_codel)。バッファ増設が輻輳を解決しないことは、RFC 970(1985)の「無限バッファのパケットスイッチ」の思考実験が早くから示していた。
- **初出章**: `07_qos/03_queuing_scheduling.md`(言及は `07_qos/01_qos_basics.md` が先行)
- **関連RFC**: RFC 8289、RFC 970
- **関連用語**: AQM、CoDel / fq_codel、テールドロップ、ジッタ
...
- **定義**: MLAG を構成する2台のスイッチを直結し、MAC 学習・メンバー状態・設定整合性の同期を運ぶリンク。平常時のデータパスにはせず(ローカル優先の転送規則)、ピアリンク経由で受けたフレームは原則 MLAG メンバーポートへ転送しない(相方側メンバーが全滅している場合のみ例外) — スプリットホライズン型のループ防止規則である。断はスプリットブレインの引き金になるため、別経路のキープアライブと必ず組で設計する。
- **初出章**: `06_redundancy/04_lag_mlag.md`
- **関連用語**: MLAG、スプリットブレイン、スプリットホライズン

### 輻輳(congestion)

- **定義**: あるリンクへ送り込みたいトラフィックが、そのリンクの送出能力を一時的に超えた状態。超過分はバッファで遅延に、バッファあふれで損失に両替される。統計多重化の設計に織り込まれた現象であり故障ではない。速度変換点(速いLAN→細いWAN)や多対一の合流点で、平均利用率と無関係にミリ秒単位で起きる(マイクロバースト)ため、5分平均の利用率グラフには写らないことが多い。
- **初出章**: `07_qos/01_qos_basics.md`
- **関連用語**: QoS、ジッタ、ベストエフォート、ブロードキャストストーム
...
- **定義**: FHRP で、より優先度の高いルータが現れたとき、現職のマスタ(Active)から役割を奪って交代すること。VRRP では既定で有効(RFC 9568。アドレスオーナー = 優先度 255 は常に奪還する)、HSRP では既定で無効。有効の網では不安定なルータの復帰のたびに切替が反復されるため、復帰後しばらく奪還を遅らせる preempt delay と組み合わせるのが定石。
- **初出章**: `06_redundancy/03_fhrp_vrrp.md`
- **関連RFC**: RFC 9568
- **関連用語**: VRRP、HSRP、FHRP
...
- **定義**: L2 ループ上でブロードキャスト等の BUM フレームがフラッディングにより複製され続け、帯域と CPU を食い尽くす状態。Ethernet フレームには TTL 相当のフィールドがなく自壊しないため、ループが物理的・論理的に断たれるまで増殖が止まらない。MAC フラッピングを伴い、短時間で網全体を無応答にする。
- **初出章**: `06_redundancy/01_stp_basics.md`
- **関連用語**: フラッディング、BUM トラフィック、MAC フラッピング、STP
...
- **定義**: 網は転送に最善を尽くすが、帯域・遅延・損失率のいずれも約束しない、という IP の基本サービスモデル。輻輳すればパケットは待たされ、バッファがあふれれば黙って捨てられる。IPv4 ヘッダには当初から品質用の座席(ToS オクテット、RFC 791。現在は DS フィールド)が用意されており、QoS はこのモデルの上に輻輳時の扱いの差別化を後付けする位置づけになる。
- **初出章**: `07_qos/01_qos_basics.md`
- **関連RFC**: RFC 791、RFC 2474
- **関連用語**: QoS、輻輳、DiffServ
...
### 優先キューイング(PQ: Priority Queuing / 厳密優先)

- **定義**: 上位キューが空でない限り下位キューに一切手を付けないスケジューラ。音声の待ち時間・ジッタの上限を「送出中の最大パケット1個分(+自クラスの先行分)」まで縮める最強の席だが、上位が絶えない限り下位が永遠に飢える(starvation)ため、原則1クラス(EF)に絞り、必ずポリシング(流入の定員)とセットで使う(RFC 3246 も EF PHB にポリシングを前提として課す)。残りのクラスは DRR/WFQ 系の重み付き分配に委ねる合成が実務の定石(Cisco の慣用名で LLQ)。
- **初出章**: `07_qos/03_queuing_scheduling.md`
- **関連RFC**: RFC 3246
- **関連用語**: EF、DRR、WFQ、QoS
...
- **定義**: 複数の物理リンクを束ねて1本の論理リンク(LAG: Link Aggregation Group)として扱う仕組み(IEEE 802.1AX。旧 802.3ad、2008 年に 802.1 へ移管)。学習・転送・フラッディング・STP のすべてから1ポートに見えるため、2点間のループが定義から消え、帯域の加算とプロトコルイベントなしの高速なリンク切替が得られる。分配はフロー単位のハッシュ(仕様の要求は「同一会話内の順序を乱さず複製しない」ことのみ)で、単一フローは1メンバーの帯域を超えられない。束は厳密に1対1であり、装置冗長には MLAG / ESI-LAG を要する。本書は「リンクアグリゲーション」「LAG」を標準表記とする(ポートチャネル、ボンディング等はベンダー慣用名)。
- **初出章**: `06_redundancy/04_lag_mlag.md`(言及は `06_redundancy/01_stp_basics.md` が先行)
- **関連用語**: LACP、MLAG、ECMP、STP
...
- **定義**: STP が張る全域木の根となるブリッジ。ブリッジ ID(プライオリティ 2 オクテット + MAC アドレス 6 オクテット)が最小の1台が選ばれる。プロトコルタイマー(Hello / Max Age / Forward Delay)はルートの設定値が BPDU で全網に配られる。既定プライオリティ(32768)のままだと MAC アドレスの偶然で決まるため、中核機のプライオリティを明示的に下げて意図的に決めるのが設計の定石。
- **初出章**: `06_redundancy/01_stp_basics.md`
- **関連用語**: STP、BPDU、ルートポート / 指定ポート

### ルートポート / 指定ポート(root port / designated port)

- **定義**: STP における転送許可ポートの2区分。ルートポートは各非ルートブリッジが持つ「ルートへの登り口」(ルートパスコスト最小の1ポート)。指定ポートは各リンク(セグメント)で「ルートに最も近い側」として選ばれた、そのリンクへ BPDU を送る責任を持つポート(ルートブリッジの全ポートは指定ポート)。どちらにも選ばれなかったポートがブロッキング(非指定ポート)となり、ループを断つ。
- **初出章**: `06_redundancy/01_stp_basics.md`
- **関連用語**: STP、BPDU、ルートブリッジ
