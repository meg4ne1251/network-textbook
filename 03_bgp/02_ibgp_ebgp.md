...
(RFC 5065)は、第3部の大規模設計の章(後述: `06_large_scale_design.md`)の
...
そのままポリシー設計の道具立てになる。意味論の詳細はパスアトリビュートの章
(後述: `03_path_attributes.md`)で扱う。
...
MPLS(第5部、後述: `../05_mpls_srv6/01_mpls_basics.md`)が
...
- 「iBGP は経路を1ホップしか運ばない」を思い出せば、
  ...
  破綻しはじめたら、それはルートリフレクタ
  (後述: `06_large_scale_design.md`)の導入時期である
