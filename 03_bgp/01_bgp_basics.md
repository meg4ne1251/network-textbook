...
eBGP/iBGP の違いは次章(後述: `02_ibgp_ebgp.md`)に委ねる。
...
制御点になる(制御の道具立ては 04 章(後述: `04_policy_control.md`)で扱う)。
...
パスアトリビュートの章(後述: `03_path_attributes.md`)の主題である。
...
- **Hold Time**: 生存判定のタイムアウト値の**提案**。双方の提案の
  **小さいほう**が採用される。値は 0(生存監視の無効化)か 3 秒以上で
  なければならない。RFC 4271 の推奨値は 90 秒だが、
...
第3部の中心テーマとして 03 章(後述: `03_path_attributes.md`)で扱う。
...
- OPEN 以前、**TCP ポート 179 の疎通の問題**である。ピアアドレスへの
  ...
  詳細は次章(後述: `02_ibgp_ebgp.md`))を順に確認する
