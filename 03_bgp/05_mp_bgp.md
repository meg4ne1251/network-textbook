...
調べてみると驚くほど少なかった**ことである。RFC 4760 は Section 1(Introduction)で
こう整理している(初版は **RFC 2283**(1998 年)、**RFC 2858**(2000 年)を経て
現行は **RFC 4760**(2007 年)):

> BGP-4 が IPv4 に固有なのは、(1) ネクストホップ(NEXT_HOP 属性、IPv4 アドレスで
> 表現される)、(2) 集約(AGGREGATOR は IPv4 アドレスを含む)、(3) NLRI(IPv4
> アドレスプレフィックスで表現される)、の3箇所だけである。
