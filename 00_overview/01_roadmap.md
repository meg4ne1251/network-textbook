...
矢印は「学習上の依存」を表す。特に重要な2本のクロス依存に注意してほしい:
...
第1部 → 第2部(01〜04章)→ 第3部 → 第2部(05章 EVPN)→ 第4部 → 第5部
...
- 本書は「なぜそう動くのか」を RFC レベルで説明できることを目指す、
  入門書卒業者向けの体系的教科書である
- 全5部構成。第1部(基礎)がすべての土台で、EVPN は BGP に、SRv6 は IPv6 に依存する
- 推奨ルートは「第1部 → 第2部前半 → 第3部 → EVPN → 第4部 → 第5部」
