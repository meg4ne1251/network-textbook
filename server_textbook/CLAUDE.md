...
公式ドキュメントレベルまで踏み込んだ、技術書として通用するレベルの内容**を目指します。
...
- Linuxサーバーの操作(コマンド、基本的な設定)には慣れているが、
  OSやカーネルが「なぜそう動くのか」を仕組みのレベルで説明できるほどではない
- 実務や趣味(ホームラボ含む)を通じて断片的な知識・勘は身についているが、
  カーネルドキュメントや仕様書レベルで動作原理を語れるほどではない
- 今後エンタープライズ〜大規模インフラの設計・運用ができる専門家を目指している

つまり「サーバーは触れるが、次の体系的な一冊が欲しい」という読者を想定する。
**個人の実務経歴(具体的な組織・案件の話)は教科書の本文には含めない。**
...
このプロジェクトは `network-guide` フォルダのサブディレクトリとして、
同じGitHubリポジトリ(`meg4ne1251/network-textbook`, public)で管理する。
...
- コミットメッセージには `server_textbook:` のプレフィックスを付け、
  `network-guide` 側のコミットと区別できるようにする
