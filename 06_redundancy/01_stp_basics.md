...
1. **物理はループを許し、論理でループを断つ** — 冗長リンクは張ったままにし、
   ...
   これが本章の主題 STP と、次章の RSTP/MSTP である。
2. **そもそもループが問題にならない構造にする** — 複数リンクを論理的に
   ...
   のようにオーバーレイで L2 を再構成する。こちらは(後述: `04_lag_mlag.md`)と、
   すでに第2部で見た世界である。
...
MSTP がこれに答える——次章で扱う)。この「遊ぶ帯域」への不満が、
リンクアグリゲーションや L3 ファブリック、そして第2部で見た
...
PortFast(ベンダー拡張)や RSTP の edge port(次章)につながる。
...
- ルートは偶然に選ばせず**プライオリティで意図して決める**。この章の
  限界(収束の遅さ・VLAN ごとの木・遊ぶ帯域)への答えが、次章の
  RSTP/MSTP と、その先のリンクアグリゲーション/L3 ファブリックである。
