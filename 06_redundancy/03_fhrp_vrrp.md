...
**Active Router** に改めた(包摂的な用語への置き換え)。この改称は
タイマー名にも及び、RFC 9568 では `Master_Down_Interval` →
**`Active_Down_Interval`**、`Master_Down_Timer` → **`Active_Down_Timer`**、
`Master_Adver_Interval` → **`Active_Adver_Interval`** となっている
(`Skew_Time` は不変)。ただし実装・コマンド体系・過去の文献では
Master/Backup が圧倒的に広く使われているため(後述の keepalived も
`state MASTER` と表記する)、本書は **マスタ(Master)/バックアップ(Backup)**
と、それに合わせた `Master_Down_Interval` 等の旧タイマー名を標準表記とし、
RFC 9568 の現行呼称をこのように併記する。
...
使い(RFC 9568 Section 5.2.9 は「仮想ルータのアドレス一覧の**先頭は
リンクローカルアドレスでなければならない**」と規定する。RA で配られる
デフォルト経路のネクストホップはリンクローカルである、という第4部03章の
話がここで効く)、マスタ(Active Router)はそのリンクローカルアドレスを
仮想ルータの RA として広告する。だからこそ切替時もホストが持つ
デフォルトルータのアドレスは変わらない。切替時に台帳を直すメッセージは、GARP の代わりに
...
- **Max Adver Int** — 送信者の広告間隔。**12 ビット、単位はセンチ秒**
  ...
  参照)。
- **Checksum** — 計算範囲はアドレスファミリで異なる。**IPv6 では
  擬似ヘッダ**(RFC 8200 Section 8.1 で定義)**を含めて**計算するが、
  **IPv4 では擬似ヘッダを含めず**、VRRP メッセージ本体(Version
  フィールドから最後の IPv4 アドレスまで)だけで計算する。後者は
  VRRPv2(RFC 3768)との後方互換のためである。なおこの点は VRRPv3 初版の
  RFC 5798 の文言が曖昧で、IPv4 に擬似ヘッダを「捏造」する実装
  (Arista EOS・FRR など)と含めない実装に分かれた歴史がある。
  現行の **RFC 9568 は「IPv4 は擬似ヘッダを含めない」と明確化**して
  この曖昧さを解消した。
